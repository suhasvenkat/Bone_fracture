...
class App(ctk.CTk):
    ...
    def predict_gui(self):
        ...
        print("Bone type result:", bone_type_result)
        
        # Check if bone type prediction failed
        if bone_type_result.startswith("Not identified:"):
            self.res1_label.configure(text_color="RED", text="Type: " + bone_type_result, font=(ctk.CTkFont("Roboto"), 20))
            self.res2_label.configure(text_color="RED", text="Result: Not identified", font=(ctk.CTkFont("Roboto"), 24))
            self.save_btn.pack(pady=10, padx=1)
            self.save_label.pack(pady=5, padx=20)
            return
        
        # If bone type is valid, proceed with fracture detection
        result = predict(filename, bone_type_result)
        print("Fracture result:", result)
        
        # Check if fracture prediction failed
        if result.startswith("Not identified:"):
            self.res1_label.configure(text="Type: " + bone_type_result, font=(ctk.CTkFont("Roboto"), 24))
            self.res2_label.configure(text_color="RED", text="Result: " + result, font=(ctk.CTkFont("Roboto"), 20))
        elif result == 'fractured':
            self.res1_label.configure(text="Type: " + bone_type_result, font=(ctk.CTkFont("Roboto"), 24))
